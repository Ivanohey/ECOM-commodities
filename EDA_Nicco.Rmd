...
Daily_futures_prices <- Daily_futures_prices[, 1:13]
Daily_futures_prices <- Daily_futures_prices[-c(1,2),]
colnames(Daily_futures_prices)[1] ="Date"
Daily_futures_prices[, 2:13] <- lapply(2:13, function(x) as.numeric(Daily_futures_prices[[x]]))
...
commo_mean <- function(set) {
  result_set <- set %>% 
    mutate(Date,
           ...
           Cocoa = rowMeans(select(., 11:13), na.rm = TRUE), .keep = "none")
