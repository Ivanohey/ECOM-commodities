...
best_lambda <- sugar_lasso_oi$lambda.min
...
list(mse = mse_lasso_oi, r2 = r2_lasso_oi, Lambda = best_lambda)


# Extract coefficients at the best lambda value
lasso_coef <- coef(sugar_lasso_oi, s = "lambda.min")
...
# Make predictions and evaluate the model
predictions <- predict(cv_model, as.matrix(test_data_scaled[, features]), s = "lambda.min")
mse <- mean((test_data_scaled[[target]] - predictions)^2)
r2 <- cor(test_data_scaled[[target]], predictions)^2

# Output the MSE, R-squared, and best lambda (alpha)
list(mse = mse, r2 = r2, best_lambda = cv_model$lambda.min)
