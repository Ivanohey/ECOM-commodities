...
# Iterate through the standardized commodity data list and perform PCA
for (commodity in commodities) {
  # Get the standardized data from the standardized list
  standardized_data <- standardized_commodity_data_list[[commodity]]
  
  # Remove "Date" column
  trading_vars <- standardized_data[, -which(names(standardized_data) == "Date")]
  
  # Perform PCA on the selected variables
  pca_result <- prcomp(trading_vars, scale. = TRUE)
...
```{r}
# Iterate through the list of PCA results and print summaries
for (commodity in commodities) {
  # Get the PCA result from the list
  pca_result <- pca_results_list[[commodity]]
  
  # Print the summary
  cat("Summary for", commodity, ":\n")
...
for (commodity in commodities) {
  ...
  # Create scree plot
  scree_plot <- plot(pca_result, type = "l", main = paste("Scree Plot -", commodity))
...
for (commodity in commodities) {
  ...
  # Create biplot
  biplot_result <- biplot(pca_result, main = paste("Biplot -", commodity))
  
  # Store the biplot in the list
  biplots_list[[commodity]] <- biplot_result
}

```
```{r}
# Create a new list to store top 5 variables for each PCA
top_variables_list <- list()

# Iterate through the PCA results
for (commodity in commodities) {
  # Get the PCA result for the current commodity
  pca_result <- pca_results_list[[commodity]]
  
  # Extract loadings for each principal component
  loadings <- pca_result$rotation
  
  # Create a matrix to store top 5 variables for each PC
  top_variables_matrix <- matrix(NA, nrow = 5, ncol = ncol(loadings))
  
  # Loop through each principal component
  for (pc in 1:5) {  # Adjust to the number of components you are interested in
    # Identify the top 5 variables with highest absolute loadings
    top_loading_variables <- names(loadings[, pc][order(-abs(loadings[, pc]))[1:5]])
    
    # Store the top variables in the matrix
    top_variables_matrix[, pc] <- top_loading_variables
  }
  
  # Store the top variables matrix in the list
  top_variables_list[[commodity]] <- top_variables_matrix
}

# Access the top 5 variables for a specific commodity and principal component
# For example, for the first commodity and PC1:
top_variables_list[["Commodity1"]][, 1]


```


We do a PCA to lower the dimensionality of the data in order to retain the most significant variables to our analysis. 
