...
Our data set is composed of 21 COT reports of commodities and an excel file containing their daily future prices. Our goal is to forecast the COT report for the week after for the following commodities: Cotton, Cocoa, Coffee and Sugar. We expect to find similarities between those commodities as they belong to the same category named "softs". This means that they are agricultural products. Nevertheless, we will add a commodity that can explain their variation: Oil. Indeed, Oil is the energy that farmers need for their machines. 

To do so, we need to understand the behavior of the variables of the COT reports independently. First, we use univariate vizualization of the open interest variable for each commodity. The open interest is known as a variable that contains a lot of information from the other COT variables. We will developed this later in our analysis.
...
```
This scatterplot seems to show a linear dependence between Coffee and Cocoa. 
...
```
From this scatterplot, Crude Oil, Cotton and Sugar seems to follow same patterns but in different proportions. 
