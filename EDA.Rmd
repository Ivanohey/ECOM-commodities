...
newdata <- COT_Cotton %>%
  select(Date, `Open Interest`, `Total Reportable Longs` , `Total Reportable Shorts` )
...
newdata2 <- COT_Cotton %>% select(Date, `Open Interest`, `Total Reportable Longs`,`Total Reportable Shorts`)
...
fit3 <- merged_data3 %>%
  model(
    ARIMA(`Open Interest` ~ `Total Reportable Longs` + `Total Reportable Shorts`))
