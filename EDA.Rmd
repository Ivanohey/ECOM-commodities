...
```{r}
# Assuming you have a list named 'pca_results_list' containing PCA results for each commodity
# and a data frame named 'commodities' with a 'commodity' column
library(factoextra)
...
```{r}
# Create a new list to store k-means clustering results
...
for (commodity in commodities) {
  ...
  # Extract the variables from the PCA result
  variables <- 
  
  # Perform k-means clustering on the variables
  kmeans_result <- kmeans(variables, centers = num_clusters)
  
  # Add cluster assignments to the original data
  commodity_data$Cluster <- as.factor(kmeans_result$cluster)
  
  # Store the k-means result in the list
  kmeans_results_list[[commodity]] <- kmeans_result
}

# Display or further process the k-means results as needed
